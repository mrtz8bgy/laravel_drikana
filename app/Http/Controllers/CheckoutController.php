    ...
    public function checkout(Request $request)
    {
        if ($request->payment_option != null) {
            ...
            if($request->session()->get('order_id') != null){
                if($request->payment_option == 'paypal'){
                ...
                }elseif($request->payment_option == 'zarinpal'){
                    $paypal = new ZarinpalController;
                    return $paypal->getCheckout();
                    ...
                    flash("خرید شما با موفقیت انجام شد.")->success();
    ...
    public function apply_coupon_code(Request $request){
        ...
        if($coupon != null){
            if(strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date){
                if(CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null){
                    ...
                    if ($coupon->type == 'cart_base')
                    {
                        ...
                        if ($sum > $coupon_details->min_buy) {
                            ...
                            flash('کد تخفیف اعمال شد')->success();
                        }
                    }
                ...
                flash('کد تخفیف منقضی شده است')->warning();
            }
        }
        else {
            flash('کد تخفیف معتبر نیست')->warning();
        }
        return back();
    }
