...
class LanguageController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $language->code = $request->code;
        if($language->save()){
            flash(__('اطلاعات زبان با موفقیت به روز رسانی شد'))->success();
            return redirect()->route('languages.index');
        }
        else{
            flash(__('Something went wrong'))->error();
    ...
    public function key_value_store(Request $request)
    {
        ...
        flash(__('ترجمه با موفقیت به روز رسانی شد').$language->name)->success();
        return back();
    }
    ...
    public function update_rtl_status(Request $request)
    {
        ...
        if($language->save()){
            flash(__('موقعیت سایت به راست چین تغییر یافت'))->success();
            return 1;
        }
        return 0;
    }
