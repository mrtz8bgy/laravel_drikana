    ...
    <div class="panel">
        ...
        <form class="form-horizontal" action="{{ route('flash_deals.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label" for="background_color">{{__('رنگ پس زمینه')}} <small>(Hexa-code)</small></label>
                    <div class="col-sm-9">
                        <input type="text" placeholder="{{__('#FFFFFF')}}" id="background_color" name="background_color" class="form-control" required>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-lg-3 control-label" for="name">{{__('رنگ متن')}}</label>
                    <div class="col-lg-9">
                        <select name="text_color" id="text_color" class="form-control demo-select2" required>
                            <option value="">انتخاب کنید</option>
                            <option value="white">{{__('سفید')}}</option>
                            <option value="dark">{{__('تیره')}}</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
