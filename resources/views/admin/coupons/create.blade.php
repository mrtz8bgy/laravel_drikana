    ...
    <div class="col-lg-8 col-lg-offset-2">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">{{__('ایجاد کد تخفیف')}}</h3>
            </div>
            ...
            <form class="form-horizontal" action="{{ route('coupon.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="panel-body">
                    <div class="form-group">
                        <label class="col-lg-3 control-label" for="name">{{__('نوع کد تخفیف')}}</label>
                        <div class="col-lg-9">
                            <select name="coupon_type" id="coupon_type" class="form-control demo-select2" onchange="coupon_form()" required>
                                <option value="">انتخاب کنید</option>
                                <option value="product_base">{{__('بر روی محصولات')}}</option>
                                <option value="cart_base">{{__('بر روی مجموع سفارش')}}</option>
                            </select>
                        </div>
                    </div>
