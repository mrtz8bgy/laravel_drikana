            ...
            <form class="" id="sort_orders" action="" method="GET">
                ...
                <div class="box-inline pad-rgt pull-left">
                    <div class="" style="min-width: 200px;">
                        <input type="text" class="form-control" id="search" name="search"@isset($sort_search) value="{{ $sort_search }}" @endisset placeholder="کد سفارش را وارد کنید و اینتر بزنید">
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <td>
                                <span class="badge badge--2 mr-4">
                                    @if ($order->orderDetails->where('seller_id',  $admin_user_id)->first()->payment_status == 'paid')
                                        <i class="bg-green"></i> پرداخت شده
                                    @else
                                        <i class="bg-red"></i> پرداخت نشده
                                    @endif
                                </span>
                            </td>
                            @if ($refund_request_addon != null && $refund_request_addon->activated == 1)
                                <td>
                                    @if (count($order->refund_requests) > 0)
                                        {{ count($order->refund_requests) }} مرجوعی 
                                    @else
                                        No Refund
                                    @endif
                                </td>
