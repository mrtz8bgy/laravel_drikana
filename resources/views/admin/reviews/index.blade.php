                ...
                <div class="box-inline pad-rgt pull-left">
                    <div class="select" style="min-width: 300px;">
                        <select class="form-control demo-select2" name="rating" id="rating" onchange="filter_by_rating()">
                            <option value="">{{__('فیلتر بر اساس امتیاز')}}</option>
                            <option value="rating,desc">امتیاز(نزولی)</option>
                            <option value="rating,asc">امتیاز(صعودی)</option>
                        </select>
                    </div>
                </div>
            </form>
    ...
    <script type="text/javascript">
        function update_published(el){
            ...
            $.post('{{ route('reviews.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'بررسی های منتشر شده با موفقیت به روز شد');
                }
                else{
                    showAlert('danger', 'مشکلی پیش آمد');
                }
            });
        }
