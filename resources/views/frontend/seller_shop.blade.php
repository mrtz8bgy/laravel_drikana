                ...
                <div class="col-md-6">
                    <ul class="text-md-left mt-4 mt-md-0 social-nav model-2">
                        @if ($shop->facebook != null)
                            <li>
                                <a href="{{ $shop->facebook }}" class="facebook social_a" target="_blank" data-toggle="tooltip" data-original-title="{{__('Facebook')}}">
                        ...
                        @if ($shop->twitter != null)
                            <li>
                                <a href="{{ $shop->twitter }}" class="twitter social_a" target="_blank" data-toggle="tooltip" data-original-title="{{__('Twitter')}}">
                        ...
                        @if ($shop->google != null)
                            <li>
                                <a href="{{ $shop->google }}" class="google-plus social_a" target="_blank" data-toggle="tooltip" data-original-title="{{__('Google')}}">
                        ...
                        @if ($shop->youtube != null)
                            <li>
                                <a href="{{ $shop->youtube }}" class="youtube social_a" target="_blank" data-toggle="tooltip" data-original-title="{{__('Youtube')}}">
                    ...
                    <div class="col-xxl-2 col-lg-3 col-md-4 col-6">
                        <div class="card product-box-1 mb-3">
                            ...
                            <div class="card-body p-0">
                                ...
                                <div class="price-bar row no-gutters">
                                    <div class="price col-md-12 text-center">
                                        @if(home_price($product->id) != home_discounted_price($product->id))
                                            <del class="old-product-price">{{ home_base_price($product->id) }}</del>
                                            <span class="product-price">{{ home_discounted_base_price($product->id) }}</span>
                                        @else
                                            <span class="product-price">{{ home_discounted_base_price($product->id) }}</span>
                                        @endif
                                    </div>
                                ...
                                <div class="cart-add d-flex text-align border-top">
                                    <button class="btn add-wishlist col-md-4" title="افزودن به علاقه مندی" onclick="addToWishList({{ $product->id }})">
                                        <i class="la la-heart-o"></i>
                                    </button>
                                    <button class="btn add-compare border-right border-left col-md-4" title="مقایسه" onclick="addToCompare({{ $product->id }})">
                                        <i class="la la-refresh"></i>
                                    </button>
                                    <button type="button" class="btn col-md-4" title="{{__('Add to cart')}}" onclick="showAddToCartModal({{ $product->id }})">
                                        <span class="d-none d-sm-inline-block"></span><i class="la la-shopping-cart"></i>
                                    </button>
                                </div>
                            </div>
                        </div>
