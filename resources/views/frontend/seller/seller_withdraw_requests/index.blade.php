                    ...
                    <div class="main-content">
                        ...
                        <div class="card no-border mt-5">
                            <div class="card-header py-3">
                                <h4 class="mb-0 h6">آرشیو تسویه حساب</h4>
                            </div>
                            <div class="card-body">
                                        ...
                                        @if(count($seller_withdraw_requests) > 0)
                                            @foreach ($seller_withdraw_requests as $key => $seller_withdraw_request)
                                                <tr>
                                                    <td>{{ $key+1 }}</td>
                                                    <td>
                                                        <input type="text" class="disabled-persian-date" id="order-date" value="{{ $seller_withdraw_request->created_at }}">
							                            <input type="hidden" id="order-date-gregorian" value="{{ $seller_withdraw_request->created_at }}">
                                                    </td>
                                                    <td>{{ single_price($seller_withdraw_request->amount) }}</td>
                                                    <td>
                                                        @if ($seller_withdraw_request->status == 1)
                                                            <span class="ml-2" style="color:green"><strong>{{__('تسویه شده')}}</strong></span>
                                                        @else
                                                            <span class="ml-2" style="color:red"><strong>{{__('در انتظار تایید')}}</strong></span>
                                                        @endif
                                                    </td>
                                                    <td>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
            <div class="modal-content position-relative">
                <div class="modal-header">
                    <h5 class="modal-title strong-600 heading-5">{{__('درخواست تسویه حساب')}}</h5>
                    ...
                    <form class="" action="{{ route('withdraw_requests.store') }}" method="post">
                        ...
                        <div class="modal-body gry-bg px-3 pt-3">
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <input type="number" class="form-control mb-3" name="amount" min="1" max="{{ Auth::user()->seller->admin_to_pay }}" placeholder="مبلغ" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-3">
                ...
                @else
                    <div class="modal-body gry-bg px-3 pt-3">
                        ...
                        شما برای ارسال درخواست تسویه ، مبلغ کافی ندارید
                        </div>
                    </div>
                @endif
            </div>
