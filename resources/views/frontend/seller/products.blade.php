                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-6 col-xl-3 mr-auto">
                                        <form class="" action="" method="GET">
                                            <input type="text" class="form-control" id="search" name="search" @isset($search) value="{{ $search }}" @endisset placeholder="جستجوی محصول">
                                        </form>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($products as $key => $product)
                                            <tr>
                                                ...
                                                <td><a class="yekan" href="{{ route('product', $product->slug) }}" target="_blank">{{ __($product->name) }}</a></td>
                                                <td>
    ...
    <script type="text/javascript">
        function update_featured(el){
            ...
            $.post('{{ route('products.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showFrontendAlert('success', 'اطلاعات با موفقیت بروزرسانی شد');
                }
                else{
                    showFrontendAlert('danger', 'مشکلی پیش آمده');
                }
            });
        }
